    ...
    with conn:
        ...
        while(cap.isOpened()):
            ...
            if best_contour[0] != 0: 
                ...
                azimuth = (x_scale * kHorizontalFOVDeg / 2.0) % 360
                kCameraPitchDeg = cv2.getTrackbarPos('Angle','Output')
                range = (kTargetHeightIn - kCameraHeightIn) / np.tan(np.radians(y_scale * (kVerticalFOVDeg / 2.0) + kCameraPitchDeg))
                print(range, azimuth, range*np.cos(np.radians(kCameraPitchDeg)), range*np.sin(np.radians(kCameraPitchDeg)))

            cv2.imshow('Output', res)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break

            conn.send(bytes(str((range,azimuth))+"\n","UTF-8"))
            time.sleep(0.01)
